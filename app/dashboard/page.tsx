...
export default function Dashboard() {
  ...
  const handleClosetFileChange = async (
          ...
          <div className="max-w-md mx-auto">
            <div className="bg-white p-8 rounded-2xl shadow-sm border-pink-100 border-2 space-y-6">
              <h2 className="text-2xl font-bold text-pink-800 flex items-center gap-2">
                <Plus className="w-6 h-6" /> Add Piece
              </h2>
              <form onSubmit={handleAddPiece} className="space-y-4">
                ...
                >
                  ...
                  <input
                    ...
                    data-testid="input-garment-file"
                  />
                </div>
                <input
                  value={label}
                ...
                <button
                  ...
                  data-testid="button-save-closet"
                >
                  {isSubmitting ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 space-y-8">
              <div className="bg-white p-6 rounded-2xl shadow-sm border-pink-100 border-2 space-y-6">
                <h2 className="text-xl font-semibold text-pink-800 flex items-center gap-2">
                  <Cloud className="w-5 h-5" /> Style Preferences
                </h2>
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="flex justify-between text-sm font-medium text-pink-700">
                      Mood <span>{mood}%</span>
                    </label>
                    <input
                      type="range"
                ...
                <button
                  ...
                  className="w-full py-4 bg-pink-600 text-white rounded-2xl font-bold shadow-lg flex flex-col justify-center items-center gap-1 hover:bg-pink-700 disabled:bg-pink-300 transition-all active:scale-95"
            ...
            <div className="lg:col-span-2 space-y-8">
              <h2 className="text-xl font-semibold text-pink-900 flex items-center gap-2">
                <Sparkles className="w-6 h-6 text-pink-600" /> AI Outfit Picks
              </h2>
              {results ? (
                <div className="space-y-6">
                  {results.map((res, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="bg-white rounded-2xl border-2 border-pink-100 shadow-sm overflow-hidden"
                        data-testid={`card-outfit-${i}`}
                      >
                          ...
                          <div className="flex-1 p-5 space-y-4">
                            <div>
                              <h3
                                ...
                                data-testid={`text-outfit-name-${i}`}
                              >
                                {res.outfitText}
                              </h3>
                              ...
                              <div>
                                ...
                                <div className="flex gap-3 flex-wrap">
                                  {selectedItems.map((item: any) => (
                                    <div
                                      key={item._id}
                                      className="flex items-center gap-2 bg-pink-50 rounded-xl p-2 pr-3"
                                      data-testid={`pill-item-${item._id}`}
                                    >
                                      ...
                                      <div>
                                        <p className="text-xs font-bold text-pink-900 leading-tight">
                                          {item.label}
                                        </p>
                ...
                <div className="bg-white p-20 rounded-2xl border-2 border-dashed border-pink-100 text-center flex flex-col items-center gap-4">
                  ...
                  <p className="text-pink-400 font-medium italic">
                    Your personalized lookbook is waiting...
                  </p>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {items?.map((item) => (
                    <div
                      ...
                      data-testid={`card-closet-${item._id}`}
                    >
                      ...
                      <div className="p-2 bg-white">
                        <p className="font-bold text-pink-900 text-[10px] truncate">
                          {item.label}
                        </p>
                        <span className="text-[8px] font-bold text-pink-400 uppercase tracking-widest">
                          {item.type}
                        </span>
                      </div>
                    </div>
